...
**Reason**: Фаза 07 добавляет поддержанный Telegram webhook transport,
поэтому прежний fail-fast стал неверным поведением.

**Migration**: При `TELEGRAM_MODE=webhook` runtime должен запускать
HTTP server, проверять Telegram secret, использовать общий router и не
запускать polling loop.
