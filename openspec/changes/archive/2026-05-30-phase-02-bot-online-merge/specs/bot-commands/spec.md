...
открыта) и отвечает приветствием с инструкцией.
В этой фазе проверки подписки **нет** — её добавит Фаза 05; сейчас
`/start` лишь знакомит пользователя с ботом. Команда идемпотентна:
повторный `/start` обновляет пользователя, а не дублирует его. Любой
**не-командный** текст в личке обрабатывается так же, как `/start`
(прощающий UX).
...
подписку
и важное замечание «писать с того же аккаунта Telegram». Текст берётся
из пакета `messages`.
...
`messages`;
inline-литералов пользовательских сообщений в коде нет (§20.3) — это
упрощает будущую локализацию. В этой фазе достаточно минимума:
приветствие, `/help` и `MSG_NO_SUB`.
