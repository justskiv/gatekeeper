...
Все user-facing admission-сообщения (запрос доступа и join-request) — продуктовые: они не раскрывают вердикты источников, internals разрешения доступа, сырые chat ID или сырые ошибки и не обвиняют пользователя в недоступности провайдера или Telegram. Временный сбой проверки описывается как временный и всегда даёт следующий шаг (повтор или кнопка); fallback на свежую подписку и ban reason в пользовательский текст как internal-причина не попадают. Контракт голоса и приватности — [bot-message-ux](bot-message-ux.md).
