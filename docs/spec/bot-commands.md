...
же grant-access flow. Каждый запрос доступа — `/start`, некомандный DM
или retry-нажатие — получает ответ: per-user throttle, схлопывавший
повторные тапы в один ответ, снят, поэтому второй `/start` или повторное
нажатие retry больше не уходят в тишину и не возвращают фейковый сбой.
