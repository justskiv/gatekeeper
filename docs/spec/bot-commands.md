...
же grant-access flow. Сюда же относится inline-кнопка «Получить доступ»,
которую `/status` показывает активному пользователю, ещё не вступившему
во все клубные ресурсы: её нажатие ведёт в тот же flow (как
retry-callback), а сам `/status` доступ не выдаёт. Каждый запрос доступа
— `/start`, некомандный DM, retry-нажатие или нажатие «Получить доступ»
— получает ответ: per-user throttle, схлопывавший повторные тапы в один
ответ, снят, поэтому второй `/start` или повторное нажатие больше не
уходят в тишину и не возвращают фейковый сбой.
Idempotency-маркер access-ответа — per-request (а не time-bucket),
поэтому каждый тап даёт свой durable reply; action-маркеры grant/invite
свою идемпотентность сохраняют. Защита от нагрузки на повторные проверки
(in-memory decision cache) отложена и в этот контракт не входит — пока
каждое нажатие запускает живую проверку.
...
`/boosty` и `/tribute` — пользовательские in-bot страницы оформления
подписки. Каждая регистрирует пользователя
(`rememberPrivateUser`/`ensureUser`) как обычный приватный вход и
отвечает durable-сообщением со ссылками на оформление соответствующей
подписки и требованием членства/канала, которое бот сверяет. Текст обеих
страниц берётся из пакета `messages`.

Это отдельный вход в подписку вместо `/start` deep-link со скрытым
payload: Telegram схлопывает deep-link до голого `/start`, который
перехватывается access-флоу, поэтому страница подписки иначе не
показывается. Видимая команда остаётся честной — без скрытого payload
(контракт UX — [bot-message-ux](bot-message-ux.md)).
...
Когда активной подписки нет, но доступ ещё сохранён на грейс-период
(есть запись `pending_revocations`), `/status` показывает тот же дедлайн
«доступ сохранён до …» и путь продления, что и проактивное уведомление,
— а не делает вид, будто доступа нет вовсе.

Когда доступ активен, но пользователь не во всех клубных ресурсах,
`/status` несёт inline-кнопку «Получить доступ» как следующий шаг.
Кнопка только инициирует grant-access flow; сам `/status` доступ не
выдаёт, а текстовый CTA «отправь /start» при наличии кнопки снимается.

Пользовательский `/status` не раскрывает внутреннюю механику решения:
сырые `AccessDecision.Reasons`, вердикты источников, `whitelist`/`system`,
локальную БД и сырые ID он не показывает — только статус, подписки,
гранты и следующий шаг (контракт голоса и приватности —
[bot-message-ux](bot-message-ux.md)).
...
записи `audit_log`. Карточка построена summary-first: сначала identity,
статус доступа, основание подписки и состояние грантов, а reasons, audit
и технические ID сгруппированы в секцию диагностики после summary. Поиск
по `@username` работает только если
...
побочный эффект — durable-доставка собственного ответа. Ответы оформлены
summary-first и отличают actionable-тревогу от рутинной сводки: рутинное
состояние читается как dashboard, а не как тревога (контракт —
[bot-message-ux](bot-message-ux.md)).
...
## Лимит сообщения и разбиение

Длинные владельческие ответы (`/whois`, `/export` и любой ответ,
способный превысить лимит Telegram) укладываются в лимит через потолок
строк или разбиение. Разбиение идёт только по границам строк и не
разрывает HTML-тег или сущность — теги держатся в пределах одной строки,
поэтому каждый фрагмент остаётся сбалансированным. CSV `/export`
отправляется как plain (без `parse_mode`), и его построчное разбиение
безопасно.
...
`/boosty` и `/tribute`. Admin scope (на `OWNER_TG_IDS`) — `/here`,
`/whois`, `/grant`,
...
будущую локализацию и ревью. Форматированные сообщения рендерятся как
Telegram HTML через централизованный renderer (экранирование динамики,
ограниченный набор тегов, безопасные ссылки — см.
[telegram-transport](telegram-transport.md)); финальный голос, граница
приватности user/owner и правила эмодзи — в
[bot-message-ux](bot-message-ux.md). Там же лежат admission-сообщения,
